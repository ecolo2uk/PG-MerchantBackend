...
const ENPAY_CONFIG = {
  ...
  merchantHashId: 'MERCOSHESYYCDAYOLFTZR8MF' // ✅ CORRECTED merchantHashId
};
...
export const generateEnpayDynamicQR = async (transactionData) => {
  try {
    ...
    const payload = {
      merchantHashId: ENPAY_CONFIG.merchantHashId, // ✅ CORRECTED
    ...
    console.log('🟡 Enpay Payload:', payload);

    const response = await enpayApi.post('/dynamicQR', payload);

    console.log('✅ Enpay Response:', response.data);
...
export const generateEnpayDefaultQR = async (transactionData) => {
  try {
    console.log('🟡 Calling Enpay Default QR API...');
